    ...
    <div class="px-4">
        @if($lastTestResult)
            <div x-data="{ visible: true }" x-show="visible" x-transition>
                <flux:callout variant="{{ $lastTestResult['variant'] }}"
                    icon="{{ $lastTestResult['variant'] === 'success' ? 'check-circle' : 'x-circle' }}">
        ...
        @if(!$endpoints->isEmpty())
            <div class="overflow-x-auto">
                <flux:table :paginate="$endpoints">
                    <flux:table.columns>
                        <flux:table.column sortable :sorted="$sortBy === 'url'"
                            :direction="$sortBy === 'url' ? $sortDirection : null" wire:click="sort('url')">
                            Endpoint URL
                        </flux:table.column>
                        <flux:table.column sortable :sorted="$sortBy === 'enabled'"
                            :direction="$sortBy === 'enabled' ? $sortDirection : null" wire:click="sort('enabled')">
                            Status
                        </flux:table.column>
                        <flux:table.column>
                            Events
                            ...
                            <flux:table.row :key="$endpoint->id">
                                <flux:table.cell variant="strong">
                                    <div class="flex flex-col">
                                        <span
                                            class="font-mono text-gray-900 font-medium truncate max-w-2xl">{{ $endpoint->url }}</span>
                                ...
                                <flux:table.cell>
                                    ...
                                    @if(count($subscribedKeys) > 0)
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            @endphp
                                            <flux:badge variant="outline" size="sm" color="zinc">{{ $metadata['label'] }}
                                            </flux:badge>
                                ...
                                <flux:table.cell>
                                    <div class="flex items-center gap-2 group/secret">
                                        <code
                                            class="rounded bg-gray-50 px-2 py-[2px] text-xs font-mono text-gray-500 ring-1 ring-inset ring-gray-200">
                                        ...
                                        <flux:button variant="ghost" size="xs" square
                                            icon="{{ ($revealedSecrets[$endpoint->id] ?? false) ? 'eye-slash' : 'eye' }}"
                                            wire:click="toggleRevealSecret({{ $endpoint->id }})" />
                                    </div>
                                </flux:table.cell>
                                <flux:table.cell align="end">
                                    <flux:dropdown position="bottom" align="end">
                                        ...
                                        <flux:menu>
                                            <flux:menu.item wire:click="testEndpoint({{ $endpoint->id }})"
                                                icon="paper-airplane">Test</flux:menu.item>
                                            <flux:menu.item wire:click="openEditModal({{ $endpoint->id }})" icon="pencil">Edit
                                            </flux:menu.item>
                                            <flux:menu.item wire:click="deleteEndpoint({{ $endpoint->id }})"
                                                wire:confirm="Are you sure you want to delete this endpoint?" icon="trash"
